...
// fs readFile
        ...
        const renderLeaderboard = (sortedData) => {
          ...
          recentArr.sort((a,b)=>b.recentSubmissions[0].timestamp-a.recentSubmissions[0].timestamp)
        //   console.log(recentArr);



        
                   let marqueue= document.getElementById('marqueue-Content')
                   
                   marqueue.innerHTML=`
                    <span > 1.${recentArr[0].name} |</span>
                    <span > 2.${recentArr[1].name} |</span>
                    <span> 3.${recentArr[2].name} |</span>
                    `
  ...
  <span style="color: green; text-decoration: underline;">${d ? d : "Unknown date"}</span>
                   ...
                   <td class="p-4 text-red-400">${student.mediumSolved && student.easySolved? student.mediumSolved/student.easySolved: 'N/A'}</td>
                                `;
...
document.getElementById('mode').addEventListener('click',()=>{
    ...
    const classM=document.getElementById('mainBody').classList
if(classM.contains('bg-gray-900')){
    document.getElementById('leader').classList.remove('bg-gray-800');
    document.getElementById('leader').classList.add('bg-white','text-black');
    
    classM.remove('bg-gray-900');
    classM.add('bg-white');
    classM.add('text-black');
}
else{
    classM.remove('bg-gray-800');
    classM.add('bg-gray-900');

    document.getElementById('leader').classList.add('bg-gray-800');
    document.getElementById('leader').classList.remove('bg-white','text-black');
